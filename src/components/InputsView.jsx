...
export default function InputsView({
	...
	const optData = useMemo(() => {
		...
		const totalCutLength = items.reduce(
			(sum, item) => sum + Number(item.length) * item.quantity,
			0,
		);
		const totalPurchased = barsNeededCount * STOCK_LIMIT;
		const efficiency =
			totalPurchased > 0 ? (totalCutLength / totalPurchased) * 100 : 0;
	...
	const handleDirectSubmit = (e) => {
		...
		onAddRow({
			...
			customDiameterAktual:
				toleranceType === "custom" ? customDiameterAktual : "",
		});
	...
	const handleSengkangSubmit = (e) => {
		...
		if (
			...
			setError(
				"Semua parameter dimensi sengkang harus diisi dengan angka positif.",
			);
			return;
		}
		...
		const cutLen = Number(
			((2 * (cleanWidth + cleanHeight) + hook * 2) / 100).toFixed(3),
		);
		...
		const nameToUse =
			sengkangName.trim() || `Sengkang D${diameter} (${width}x${height})`;
	...
	const liveDirectPreview = useMemo(() => {
		...
		if (isTolerance) {
			...
			if (diameterAktual > 0) {
				...
				shrinkagePct = Number(
					(
						((nominalWeightFactor - actualWeightFactor) / nominalWeightFactor) *
						100
					).toFixed(2),
				);
			}
		...
		const weightActualKg =
			0.006165 * diameterAktual * diameterAktual * finalLength * N;
		...
		<div className="inputs-layout-grid">
			...
			<div className="form-inputs-container">
				{!hasActiveProject ? (
					...
					>
						...
						>
							Silakan pilih proyek yang sudah ada atau buat proyek baru terlebih
							dahulu di menu **Projects** sebelum Anda dapat menginput data
							pemotongan rebar.
						</p>
						<button
							...
							{formType === "direct" ? (
								<form onSubmit={handleDirectSubmit} className="form-grid">
									<div className="form-group full-width">
										<label htmlFor="elementName">
											Nama Elemen / Deskripsi (Opsional)
										</label>
										<input
											id="elementName"
											type="text"
									...
									<div className="form-group full-width">
										<label htmlFor="cutLength">Panjang potongan (L)</label>
									...
									<div className="form-group full-width">
										<label htmlFor="quantity">Jumlah potongan (N)</label>
										<div className="input-group">
											<input
												id="quantity"
												type="number"
									...
									>
										...
										{hasOverlap && (
											...
											>
												...
												>
													<option value="40d">
														Tulangan Bawah (Tarik) - 40d
													</option>
													<option value="50d">
														Tulangan Atas (Tarik) - 50d
													</option>
												...
												{overlapType === "custom" && (
													<input
														...
														onChange={(e) =>
															setCustomOverlapFactor(e.target.value)
														}
														style={{
															padding: "0.5rem",
															fontSize: "0.85rem",
											...
											>
												...
												{toleranceType === "custom" && (
													<input
														...
														onChange={(e) =>
															setCustomDiameterAktual(e.target.value)
														}
														style={{
															padding: "0.5rem",
															fontSize: "0.85rem",
									...
									{liveDirectPreview && (
										<div
											className="sengkang-live-preview-banner"
											style={{
												gridColumn: "span 2",
												display: "flex",
												flexDirection: "column",
										...
										>
											{hasOverlap && (
												...
												>
													Overlap:{" "}
													<strong>+{liveDirectPreview.overlapLength} m</strong>{" "}
													per batang (Total: {liveDirectPreview.finalLength} m)
												</div>
											)}
											{isTolerance && (
												<div
												...
												>
													Besi Banci ({liveDirectPreview.diameterAktual}mm):
													Penyusutan Berat{" "}
													<strong>{liveDirectPreview.shrinkagePct}%</strong>
												</div>
											)}
											<div
												className="preview-sub-text"
												style={{ fontSize: "0.75rem", fontWeight: "600" }}
											>
												Berat Nominal:{" "}
												{liveDirectPreview.weightNominalKg.toFixed(2)} kg
												{isTolerance &&
													` | Berat Timbangan: ${liveDirectPreview.weightActualKg.toFixed(2)} kg`}
											</div>
										</div>
									)}
								...
								<form onSubmit={handleSengkangSubmit} className="form-grid">
									...
									{liveSengkangPreview && (
										...
										>
											<div className="preview-main-text">
												Total Kebutuhan:{" "}
												<strong>{liveSengkangPreview.quantity} Pcs</strong>{" "}
												Sengkang ({liveSengkangPreview.weightKg.toFixed(2)} kg)
											</div>
											<div className="preview-sub-text">
												Besi {steelType === "BjTP" ? "Polos" : "Ulir"} D
												{diameter} &bull; Panjang 1 Begel:{" "}
												{liveSengkangPreview.singleLength}m
											</div>
										</div>
									)}
									...
									<button
										type="submit"
										className="btn btn-primary full-width"
										style={{ marginTop: "0.5rem", gridColumn: "span 2" }}
									>
										<svg
											fill="none"
											viewBox="0 0 24 24"
						...
						<div className="card">
							...
							<div className="preset-badge-group">
								<button
									...
									onClick={() => applyPreset(10, 2.5)}
								>
									D10 - 2.5M
								</button>
								<button
									className="preset-badge"
									onClick={() => applyPreset(13, 4.0)}
								>
									D13 - 4.0M
								</button>
								<button
									className="preset-badge"
									onClick={() => applyPreset(16, 3.2)}
								>
									D16 - 3.2M
								</button>
							</div>
					...
					<div className="pattern-visualization-list">
						{optData.patterns.length > 0 ? (
							optData.patterns.map((p, pIdx) => {
								const totalPieceLength = p.pieces.reduce(
									(sum, item) => sum + item.length,
									0,
								);
								const scrapLength = Number(
									(12.0 - totalPieceLength).toFixed(2),
								);
								return (
									<div key={p.key} className="pattern-row-visual">
										<div className="pattern-row-header">
											<span>
												POLA {pIdx + 1} (x{p.count} batang stock - D{p.diameter}
												)
											</span>
											{scrapLength > 0 ? (
												<span className="text-danger font-bold">
													SISA: {scrapLength}M (SCRAP)
												</span>
											) : (
										...
										<div className="pattern-bar-container">
											{p.pieces.map((piece, pieceIdx) => {
												...
												const classToUse =
													pieceClasses[pieceIdx % pieceClasses.length];
												return (
													<div
														key={pieceIdx}
							...
							<div className="empty-state" style={{ padding: "2rem 1rem" }}>
								<p style={{ color: "#9ca3af", fontSize: "0.9rem" }}>
									Belum ada pola optimasi. Silakan tambahkan data potongan di
									form sebelah kiri.
								</p>
							</div>
						)}
										...
										<tr key={item.id}>
											...
											<td>
												...
												>
													...
													{item.hasOverlap &&
														` | Overlap +${item.overlapLength}m`}
													{item.isTolerance &&
														` | Banci (${item.diameterAktual}mm)`}
												</div>
											</td>
			...
			{hasActiveProject && (
				<button
					...
					onClick={
						formType === "direct" ? handleDirectSubmit : handleSengkangSubmit
					}
				...
				<div className="modal-overlay" onClick={() => setItemToDelete(null)}>
					<div className="modal-content" onClick={(e) => e.stopPropagation()}>
						...
						>
							...
							<strong>{itemToDelete.elementName}</strong> (D
							{itemToDelete.diameter} x {itemToDelete.length}m)?
						</p>
			...
			{showClearConfirm && (
				...
				>
					<div className="modal-content" onClick={(e) => e.stopPropagation()}>
						...
						>
							Apakah Anda yakin ingin menghapus **semua data potongan aktif**
							dalam proyek ini? Tindakan ini tidak dapat dibatalkan.
						</p>
